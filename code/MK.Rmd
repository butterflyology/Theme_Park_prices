...
The steepness of the lines is what we are interested in. Are the slopes between Michael Eisner and Bob Iger the same? They don't appear to be the same but I want to check. We will use a procedure called `ANCOVA` or **AN**alysis of **COVA**riance. 
...
We fit the model without the interaction term (`Date + CEO`) to ask if CEO has a significant effect on the dependant variable. 
