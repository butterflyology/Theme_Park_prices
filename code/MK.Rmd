...
Note that `readr` added the "day" to each observation in the `Date` columns. I didn't tell it to do that, it is just the way the program is. Apparently, the world will end if you don't have a day in your date object. I don't truly know the dates that the prices were in effect but it won't matter for our analysis, but it is frustrating that the program added data.
...
It really does look like Mr. Iger has upped the rate of admission increases, doesn't it? This is nice and all but isn't as informative as we need for a meaningful comparison. We need to adjust the prices for inflation. We will adjust inflation using the "Consumer Price Index for All Urban Consumers: All Items."
...
p <- xts(data$Price, data$Date)
colnames(p) <- "price"
head(p)

avg.cpi <- apply.yearly(CPIAUCSL, mean)
cf <- avg.cpi / as.numeric(avg.cpi['1971'])
...
# Now multiply the 1973 amount times the CPIAUCSL number
```
...
# initialize a blank column to hold the data
adj_val <- matrix(nrow = length(years), ncol = 2)
colnames(adj_val) <- c("Year", "Avg_adj")
...
Are the slopes between Michael Eisner and Bob Iger the same? Earlier we used a linear regression to find lines that best describe each set of admission prices by CEO. We will ask if the amount of the price increase was significantly different between these CEO
